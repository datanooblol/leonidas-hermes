...
export const JourneyGuide = ({ currentStage, stages, onStageChange, guide }: JourneyGuideProps) => {
  const defaultGuide = {
    ...
    lines_to_say: ["Please wait while we connect to the system"],
    signals: []
  };
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className="bg-white dark:bg-[#1E1F20] rounded-xl border-l-4 border-l-[#0B57D0] border-y border-r border-gray-200 dark:border-[#444746] p-6 shadow-sm relative overflow-hidden group">
                 ...
                 <div className="flex items-center justify-between mb-4 relative z-10">
                   ...
                   <div className="flex gap-2">
                       {(currentGuide.signals || []).map((signal: string, idx: number) => (
                         <span key={idx} className="px-2.5 py-1 bg-gray-100 dark:bg-[#2D2E30] border border-gray-200 dark:border-[#444746] rounded-md text-[10px] font-semibold text-gray-600 dark:text-gray-300">
                           {signal}
                         </span>
                       ))}
                   </div>
              ...
              <div className="flex flex-col">
                  ...
                  <div className="space-y-3">
                    {(currentGuide.lines_to_say || []).map((line: string, idx: number) => (
                      <div 
                        key={idx} 
