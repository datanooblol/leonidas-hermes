...
class TranscriptionTask:
    ...
    async def trigger_queues(self):
        ...
        messages_to_send = await self.should_trigger_queue(self.transcription_messages, window_size=6, offset=2, start_at=6)
        if messages_to_send:
            self.logger.debug(f"Triggering stage queue: {messages_to_send}")
